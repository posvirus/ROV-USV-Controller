...
struct data//数据结构体
{
  ...
  char latitude[20];//纬度
  double anglez;//航向角，航向角的取值规则如下，以正北为0度
...
void EVENT_motivate()
{
  ...
  if (module == '1')
  {
    ...
    if (!working_flag)//如果当前没有需要到达的结点
    {
      ...
      {
        present_target.anglez = 180.0 - atan((present_target.X() - present_location.X()) / (present_location.Y() - present_target.Y())) / PAI * 180.0;
      }
      else if (present_target.X() < present_location.X() && present_target.Y() < present_location.Y())
      {
        present_target.anglez = 180.0 + atan((present_location.X() - present_target.X()) / (present_location.Y() - present_target.Y())) / PAI * 180.0;
      }
      else
      {
        present_target.anglez = 360.0 - atan((present_location.X() - present_target.X()) / (present_target.Y() - present_location.Y())) / PAI * 180.0;
      }
    ...
    { //如果当前航向角与目标航向角的差值大于MAX_ANGLE_ERR，先进行转向
      //Serial3<<"GET"<<endl;
      if (present_location.anglez > present_target.anglez && present_location.anglez - present_target.anglez < 180.0)//逆时针
      {
        v_o_m[2] = TURN_LBM;
        v_o_m[3] = TURN_RBM;
      }
      else if (present_location.anglez > present_target.anglez && present_location.anglez - present_target.anglez >= 180.0) //顺时针
      {
        v_o_m[2] = TURN_RBM;
        v_o_m[3] = TURN_LBM;
      }
...
void location_update()
{
  ...
  */
  if (Serial1.peek() == 'X')
  {
    ...
    result = Serial1.readBytes(angle, 6);
    present_location.anglez = (angle[0] - '0') * 100.0 + (angle[1] - '0') * 10.0 + (angle[2] - '0') * 1.0 + (angle[4] - '0') * 0.1 + (angle[5] - '0') * 0.01;
